...
def get_user(email):
    return requests.get(f"{API}/get_user", params={"email": email}).json()

def save_user(user):
    return requests.post(f"{API}/save_user", json=user).json()

def get_all_products():
    return requests.get(f"{API}/get_all_products").json()

def get_product_by_name(name):
    return requests.get(f"{API}/get_product_by_name", params={"name": name}).json()

def get_confirmation(conf_id):
    return requests.get(f"{API}/get_confirmation", params={"id": conf_id}).json()

def save_confirmation(conf_id, email):
    return requests.post(f"{API}/save_confirmation", json={"id": conf_id, "email": email}).json()

def delete_confirmation(conf_id):
    return requests.delete(f"{API}/delete_confirmation", params={"id": conf_id}).json()
...
try:
    ...
    logging.error(f"Erro ao ligar ao MQTT: {e}")

@app.route('/weather/rest')
def weather_rest():
    response = requests.get('https://cjsg.ddns.net:8443/weather/values', verify=False)
    return response.json()

@app.route('/weather/mqtt')
def weather_mqtt_route():
    return weather_mqtt
